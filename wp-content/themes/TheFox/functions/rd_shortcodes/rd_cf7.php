...
function rd_cf7($atts, $content = null) {
	...
	$output = '<style>';
	if($text_color !== ''){
	$output .='#rd_'.$cf_id.' .wpcf7,#rd_'.$cf_id.' input:not([type=submit]),#rd_'.$cf_id.' textarea,#rd_'.$cf_id.' .wpcf7 select,#rd_'.$cf_id.' .wpcf7 input[type="checkbox"]:checked{color:'.$text_color.';}#rd_'.$cf_id.' .wpcf7 input[type="checkbox"]{background:rgba(0,0,0,0); color:rgba(0,0,0,0);}';
	}
	if($place_holder !== ''){
